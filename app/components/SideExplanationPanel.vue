...
<template>
  <div class="w-full bg-zinc-900/40 border border-zinc-800 p-4 flex flex-col select-none">
    ...
    <div v-if="!activeComplexHint" class="flex flex-col items-center justify-center text-center p-6 py-10">
      ...
      <h3 class="text-sm 3xl:text-base font-bold text-zinc-400 uppercase tracking-wider">{{ $t('hint.panelTitle') }}</h3>
    ...
    <div v-else class="flex flex-col gap-4">
      ...
      <div class="border-b border-zinc-800 pb-3">
        ...
        <h2 class="text-lg 3xl:text-2xl font-black text-zinc-100 tracking-tight leading-tight">{{ activeComplexHint.title }}</h2>
      </div>
      ...
      <div v-if="currentStep" class="bg-zinc-950/60 border border-zinc-800 p-4 space-y-2">
        ...
        <p class="text-sm 3xl:text-base text-zinc-300 leading-relaxed">
          {{ currentStep.description }}
        </p>
      </div>

      <!-- Legend -->
      <div class="border-t border-zinc-800/60 pt-3 space-y-1.5 text-xs 3xl:text-sm font-medium text-zinc-500">
      ...
      <div class="flex flex-col gap-2 border-t border-zinc-800 pt-3">
        <div class="grid grid-cols-2 gap-2">
          <button
            ...
            class="py-2.5 3xl:py-3 bg-zinc-900 border border-zinc-800 text-xs 3xl:text-sm font-bold uppercase transition-all active:scale-95"
          >
            {{ $t('hint.prev') }}
          </button>
