...
function caption(id: TechniqueId, ex: AcademyExample): string {
  ...
  switch (id) {
    ...
    case 'swordfish':
    case 'jellyfish': {
      const lines = t(`academy.lines.${id === 'x-wing' ? 'two' : id === 'swordfish' ? 'three' : 'four'}`);
  ...
  <div class="min-h-screen w-full flex flex-col">
    ...
    <div class="sticky top-0 z-10 backdrop-blur border-b pl-4 sm:pl-8 pr-4 py-4 flex items-center gap-4 dark:bg-[#0c0a09]/95 dark:border-zinc-800 bg-white/95 border-zinc-200">
      ...
      <div class="flex-1 min-w-0">
        <h1 class="text-lg sm:text-2xl font-black tracking-tight leading-tight dark:text-zinc-100 text-zinc-900">{{ $t('academy.title') }}</h1>
        <p class="text-xs text-zinc-500 hidden sm:block">{{ $t('academy.subtitle', { n: total }) }}</p>
      </div>
      <span class="shrink-0 text-xs text-zinc-600 font-semibold hidden sm:inline">{{ $t('academy.count', { n: total }) }}</span>
    </div>
    ...
    <div class="flex-1 px-4 sm:px-8 py-6 sm:py-8 max-w-5xl mx-auto w-full">
      ...
      <div class="flex flex-wrap items-center gap-x-4 gap-y-2 mb-6 sm:mb-8 px-3 sm:px-4 py-3 border text-[11px] dark:bg-zinc-900/40 dark:border-zinc-800 dark:text-zinc-400 bg-zinc-50 border-zinc-200 text-zinc-600">
        <span class="font-bold uppercase tracking-widest dark:text-zinc-300 text-zinc-700">{{ $t('academy.colors') }}</span>
        <span class="flex items-center gap-1.5"><span class="w-3 h-3 bg-emerald-500/30 ring-1 ring-emerald-500 inline-block"></span> {{ $t('academy.placedCell') }}</span>
        <span class="flex items-center gap-1.5"><span class="w-3 h-3 bg-indigo-500/40 ring-1 ring-indigo-400 inline-block"></span> {{ $t('academy.pattern') }}</span>
        <span class="flex items-center gap-1.5"><span class="w-3 h-3 bg-rose-500/40 ring-1 ring-rose-400 inline-block"></span> {{ $t('academy.eliminated') }}</span>
      ...
      <div v-for="tier in tiers" :key="tier" class="mb-10 sm:mb-12">
        ...
        <div class="flex items-center gap-3 mb-2">
          <h2 :class="tierHeading[tier]" class="text-lg sm:text-xl font-black uppercase tracking-widest">{{ $t(`academy.tier.${tier}`) }}</h2>
          <span :class="tierBadgeClass[tier]" class="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 border">
            {{ byTier(tier).length }}
          </span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            <div class="flex items-start justify-between gap-3 mb-3">
              <h3 class="text-base font-black leading-tight dark:text-zinc-100 text-zinc-900">{{ techName(tech.id) }}</h3>
            ...
            <div class="space-y-3">
              <div>
                <p class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">{{ $t('academy.lookForLabel') }}</p>
                <p class="text-sm leading-relaxed dark:text-zinc-300 text-zinc-700">{{ $t(`academy.tech.${tech.id}.lookFor`) }}</p>
              </div>
              <div>
                <p class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">{{ $t('academy.howItHelpsLabel') }}</p>
                <p class="text-sm leading-relaxed dark:text-zinc-400 text-zinc-600">{{ $t(`academy.tech.${tech.id}.howItHelps`) }}</p>
              </div>
            </div>
            ...
            <div v-if="expanded.has(tech.id) && exampleFor(tech.id)" class="mt-4 pt-4 border-t dark:border-zinc-800 border-zinc-200" @click.stop>
              <p class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3 text-center">{{ $t('academy.workedExample') }}</p>
              <ExampleGrid
      ...
      <div class="border-t pt-8 text-center dark:border-zinc-800 border-zinc-200">
        <p class="text-xs text-zinc-600 leading-relaxed max-w-lg mx-auto">{{ $t('academy.footer') }}</p>
        <button
          @click="emit('back-to-menu')"
