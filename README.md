...
> *Highlighted in blue: all cells that block the other candidates.*
...
Even if the target cell has multiple candidates, you can find where a specific number *must* go by scanning an entire unit (row, column, or box) and seeing that no other cell in that unit accepts it.

> *Highlighted in blue: the filled cells that rule out the digit elsewhere in the unit.*
...
> *Blue: the pair cells. Red: cells where the pair's digits can be eliminated.*
...
> *Blue: the aligned candidate cells inside the box. Red: eliminations outside the box.*
...
> *Blue: the four rectangle corners. Red: column eliminations outside the two rows.*
